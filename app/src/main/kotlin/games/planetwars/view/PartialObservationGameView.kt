...
class PartialObservationGameView(
    var gameState: GameState,
    val params: GameParams = GameParams(),
    val colors: ColorScheme = ColorScheme(),
    var gameRunner: PartialObservationGameRunner? = null,
...
//        Player.Neutral,
    ...
    private fun drawPlanets(xg: XGraphics) {
        for (planet in gameState.planets) {
            ...
            // In partial observation, show ship counts only for planets visible to the observing players
            if (planet.owner in showInfoFor) {
                val tStyle = TStyle(fg = colors.text, size = 14.0)
                val text = XText("${planet.nShips.toInt()}", planet.position, tStyle)
                xg.draw(text)
            } else {
                // For unowned planets in partial observation, show "?" to indicate hidden information
                val tStyle = TStyle(fg = colors.text, size = 14.0)
                val text = XText("?", planet.position, tStyle)
                xg.draw(text)
            }
        }
    }
    ...
    private fun drawTransporter(xg: XGraphics, transporter: Transporter) {
        ...
        // In partial observation, show ship counts only for transporters owned by observing players
        if (transporter.owner in showInfoFor) {
            val tStyle = TStyle(fg = colors.text, size = 14.0)
            val text = XText("${transporter.nShips.toInt()}", transporter.s, tStyle)
            xg.draw(text)
        } else {
            // For enemy transporters in partial observation, show "?" to indicate hidden information
            val tStyle = TStyle(fg = colors.text, size = 12.0)
            val text = XText("?", transporter.s, tStyle)
            xg.draw(text)
        }
    }
    ...
    private fun drawStatus(xg: XGraphics) {
        ...
        val tStyle = TStyle(fg = colors.text, size = 14.0)
        val text = XText("Game status (Partial Observation): $status", Vec2d(width / 2, 20.0), tStyle)
        xg.draw(text)
    }
    ...
    override fun handleKeyEvent(e: XKeyEvent) {
        ...
        if (e.keyCode == 'F'.code) {
            AppLauncher.globFrameRate *= 2.0
            // limit the frame rate to 100fps
