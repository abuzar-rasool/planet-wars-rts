...
class StrategicAgentDynamic(
    ...
    // Dynamically determined maxHorizon
    private var dynamicMaxHorizon: Int = 100 // Default, will be calculated
    ...
    private fun evaluatePosition(gameState: GameState): Double {
        ...
        for (myPlanet in myPlanets) {
            val nearestEnemyDist = oppPlanets.minOfOrNull { distanceBetween(myPlanet, it) } ?: Double.MAX_VALUE
            ...
            if (myPlanets.size > 1) {
                val nearestFriendDist = myPlanets.filter { it.id != myPlanet.id}
                                           .minOfOrNull { distanceBetween(myPlanet,it) } ?: Double.MAX_VALUE
                if (nearestFriendDist > params.width / 4.0) { // Heuristic for "too far"
